...
def balance(request, pk):
    ...
    transactions = Transaction.Transaction.filter(account=pk)  # Retrieve all of that account's transaction
    ...
    for t in transactions:  # Loop through transaction and determine which is a deposit or withdrawal
        if t.type == 'Deposit':
            ...
            table_contents.update({t: current_total})  # Add transaction and total to the dictionary
            # Pass account, account total balance, and transaction information and total to the template
    content = {'account': account, 'total_content': table_contents, 'balance': current_total}
...
def transaction(request):
    ...
    if request.method == 'POST':
        if form.is_valid():  # Check to see if the submitted form is valid and if so, save the form
            ...
            return balance(request, pk)  # Renders balance of the account Balance Sheet
    content = {'form': form}  # Saves content to the template in a dictionary
    # adds content of form to page
